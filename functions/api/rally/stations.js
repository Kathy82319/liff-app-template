...
export async function onRequest(context) {
    ...
    try {
        ...
        if (method === 'POST') {
            ...
            if (codeCheck) {
                 return new Response(JSON.stringify({ error: `夥伴代碼 "${final_code}" 已存在，請更換或重新嘗試。` }), { status: 409 });
            }
            ...
            return new Response(JSON.stringify({ success: true, station: result }), { status: 201 });
        }

        // --- PUT: 更新站點 ---
        if (method === 'PUT') {
            if (!station_id) errors.push('缺少 station_id。');
            if (errors.length > 0) {
                 return new Response(JSON.stringify({ error: errors.join(' ') }), { status: 400 });
            }
            ...
            if (final_code) {
                 ...
                 if (codeCheck) {
                     return new Response(JSON.stringify({ error: `夥伴代碼 "${final_code}" 已被其他站點使用。` }), { status: 409 });
                 }
            }
            ...
            return new Response(JSON.stringify({ success: true, message: '站點更新成功' }), { status: 200 });
        }

        // --- DELETE: 刪除站點 ---
        if (method === 'DELETE') {
            const { station_id: delete_id } = body;
            if (!delete_id) return new Response(JSON.stringify({ error: '缺少 station_id' }), { status: 400 });
            ...
            return new Response(JSON.stringify({ success: true, message: '站點與所有相關進度資料已刪除' }), { status: 200 });
        }
