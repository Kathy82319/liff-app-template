...
export async function init() {
    ...
    const isListView = document.getElementById('booking-view-list')?.classList.contains('active');
    if (isListView) {
        loadOrderList();
    } else {
        if (!state.flatpickrInstance) {
            initializeCalendar(); // 假設此函式在同一檔案內
        } else {
            // 如果切換回來，重新載入一下
...
async function loadOrderList() {
    ...
    try {
        ...
        container.innerHTML = items.map(item => {
             ...
             if (state.currentTemplate === 'studio_template' || state.currentTemplate === 'guesthouse_template') {
                 type = 'booking';
                 id = item.booking_id;
            ...
            // 新的卡片 HTML 結構 (左右佈局)
            // 注意：ui.translateStatus 需從 ui 模組引入
            return `
                ...
                <div class="order-card-layout">
                    ...
                    <div class="order-card-right">
                        <span class="status-badge-text" style="color: ${statusColor}; background: ${statusBg};">
                            ${ui.translateStatus(item.status)}
                        </span>
                    </div>
                </div>
            </div>`;
...
// --- [新增] 設定預設日期 (當月第一天 ~ 當月最後一天) ---
function setDefaultDateRange() {
    const startInput = document.getElementById('order-date-filter-start');
    const endInput = document.getElementById('order-date-filter-end');
    
    // 如果欄位存在且是空的才填入，避免覆蓋使用者的選擇
