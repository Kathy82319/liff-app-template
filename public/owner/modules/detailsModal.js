...
// ... (openDetailsModal, renderBookingDetailsBody, renderCustomerDetailsBody 保持不變，與上個回答相同) ...
// 為了節省篇幅，這部分請使用上一個回答中的程式碼，或者如果您需要完整檔案請告訴我
...
export function renderBookingDetailsBody(details) {
     ...
     if (itemList.length > 0) {
         itemList.forEach(item => { html += `<p>- ${item.item_name} x ${item.quantity} ($${item.price || 0})</p>`; });
     } else { html += '<p>無項目資料</p>'; }
...
export function renderCustomerDetailsBody(data) {
     ...
     if (rally_progress_summary && rally_progress_summary.length > 0) {
         rallyHtml = rally_progress_summary.map(r => {
             ...
             return `<div style="background: var(--color-bg); padding: 8px; border-radius: 4px; border: 1px solid var(--color-secondary); margin-bottom: 8px;"><div style="display: flex; justify-content: space-between;"><span style="font-weight: bold;">${r.title}</span><span style="${statusStyle}">${r.collected} / ${r.required} 點</span></div></div>`;
         }).join('');
     }
     ...
     if (showVouchers) {
         ...
         if (activeVouchers.length > 0) {
             vouchersHtml = activeVouchers.map(v => `<div style="background: var(--color-bg); padding: 8px; border-radius: 4px; margin-bottom: 5px; border: 1px solid var(--color-secondary);"><span style="color: var(--color-success); font-weight: bold;">●</span> ${v.title}</div>`).join('');
         } else { vouchersHtml = '<p style="color:var(--color-text-secondary);">無可用優惠券</p>'; }
     }
     ...
     if (bookings && bookings.length > 0) {
         bookingsHtml = bookings.slice(0, 3).map(b => {
             ...
             return `<div style="background: var(--color-bg); border: 1px solid var(--color-secondary); border-radius: 6px; padding: 10px; margin-bottom: 8px;"><div style="display: flex; justify-content: space-between;"><span style="font-weight: bold;">${time}</span><span>${ui.translateStatus(b.status)}</span></div></div>`;
         }).join('');
     }
...
export function renderCustomerActions(profile) {
      ...
      let buttons = [];
      if (features.OWNER_CRM_SHOW_STORED_VALUE !== false) buttons.push(`<button class="cta-button" data-action="adjust-stored-value" data-user-id="${profile.user_id}" data-target-name="${targetName}" style="background-color: var(--color-success);">儲值/扣款</button>`);
      if (features.OWNER_CRM_SHOW_VOUCHERS !== false) buttons.push(`<button class="cta-button" data-action="issue-voucher" data-user-id="${profile.user_id}" data-target-name="${targetName}" style="background-color: var(--color-info);">發送優惠券</button>`);
...
async function openSendMessageModal(targetUserId, targetName) {
    ...
    const contentTextarea = document.getElementById('direct-message-content');
    ...
    newBtn.onclick = async () => {
        const msg = contentTextarea.value.trim();
        ...
        try {
            ...
            ui.toast('發送成功');
            ui.updateHistoryState('send-message', 'close'); 
