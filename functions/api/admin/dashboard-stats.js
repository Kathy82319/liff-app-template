...
export async function onRequest(context) {
  try {
    ...
    const statements = [
      ...
      // 4. 【關鍵修正】熱門服務排行 - 讓 SQL 自己計算當前月份
      db.prepare(`
        SELECT item_name, SUM(quantity) as total_quantity 
        FROM BookingItems 
        WHERE booking_id IN (SELECT booking_id FROM Bookings WHERE strftime('%Y-%m', booking_date) = strftime('%Y-%m', 'now', 'localtime'))
        GROUP BY item_name 
