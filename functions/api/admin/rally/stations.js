...
export async function onRequest(context) {
    ...
    try {
        ...
        if (method === 'PUT') {
            ...
            // 驗證邏輯 (僅針對 PUT)
            if (!name || name.trim().length === 0) return new Response(JSON.stringify({ error: '站點名稱為必填。' }), { status: 400, headers: jsonHeaders });
        ...
        // --- DELETE: 刪除站點 (防呆機制) ---
        if (method === 'DELETE') {
            // 【關鍵】DELETE 只需要 station_id 和 force，不需要其他驗證
        ...
        return new Response(JSON.stringify({ error: '處理集點站點失敗', details: error.message }), { status: 500, headers: jsonHeaders });
    }
}