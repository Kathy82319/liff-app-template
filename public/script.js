...
// 【修正2】使用 history API 處理上一頁
// 頁面導航函式
function navigateTo(pageId, stateData = {}) {
    // 隱藏所有頁面
    ...
    // 更新瀏覽器歷史紀錄
    // pageId 當作 state 的一部分，方便 popstate 事件處理
    const finalState = { page: pageId, ...stateData }; 
    // # 後面的 hash 只是為了讓 URL 看起來更直觀，實際功能由 state 控制
    const hash = pageId.replace('-view', ''); 
    history.pushState(finalState, '', `#${hash}`);
}

// 處理瀏覽器的返回操作 (例如：點擊手機的返回鍵)
window.addEventListener('popstate', (event) => {
    // 如果 state 為空，表示已退到最開始，可能需要關閉 liff
    if (!event.state) {
        // 在某些情況下，LIFF App 會在退無可退時自動關閉
        // 這裡可以選擇顯示首頁
        showPage('home-view');
        return;
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const pages = {
        ...
        catalog: document.getElementById('product-catalog-view'),
        detail: document.getElementById('product-detail-view'),
        bookings: document.getElementById('booking-history-view'),
        profile: document.getElementById('my-profile-view')
    };
    ...
    try {
        ...
        // 設置初始頁面歷史狀態
        history.replaceState({ page: 'home-view' }, '', '#home');
    ...
    // --- 導航按鈕事件監聽 ---
    document.querySelector('button[data-page="product-catalog-view"]').addEventListener('click', () => navigateTo('product-catalog-view'));
    document.querySelector('button[data-page="booking-history-view"]').addEventListener('click', () => navigateTo('booking-history-view'));
    
    // 所有返回按鈕都使用 history.back()
...
async function fetchProductCatalog() {
    try {
        ...
        catalogContainer.innerHTML = ''; // 清空舊內容
        
        products.filter(p => p.is_visible).forEach(p => {
            const imageUrl = p.image_urls && p.image_urls.length > 0 ? p.image_urls[0] : 'https://via.placeholder.com/300';
            ...
            `;
            card.addEventListener('click', () => showProductDetail(p.product_id, products));
...
// 顯示產品詳細資訊
function showProductDetail(productId, allProducts) {
    const product = allProducts.find(p => p.product_id === productId);
    if (!product) return;

    const imageSlider = document.getElementById('product-detail-images');
    const detailName = document.getElementById('product-detail-name');
    const detailDesc = document.getElementById('product-detail-desc');
    ...
    productSpecs.innerHTML = ''; // 清空舊規格
    if (product.specifications) {
        try {
            const specs = JSON.parse(product.specifications);
            specs.forEach(spec => {
                if(spec.name && spec.value) { // 確保規格有名稱和值
            ...
            console.error('解析產品規格失敗', e);
        }
    }
    
    // 【修正2】導航至詳細頁面
    navigateTo('product-detail-view', { productId: productId });
}
...
async function fetchBookingHistory() {
    ...
    try {
        ...
        if (!response.ok) throw new Error('無法獲取預約歷史');
        ...
        if (bookings.length === 0) {
            historyList.innerHTML = '<p>您目前沒有任何預約記錄。</p>';
            return;
        }
        ...
        bookings.forEach(booking => {
            ...
            const formattedDate = bookingDate.toLocaleString('zh-TW', {
            ...
            item.innerHTML = `
                ...
                <p><strong>人數:</strong> ${booking.people_count} 人</p>
                <p><strong>狀態:</strong> ${booking.status === 'cancelled' ? '已取消' : '已確認'}</p>
            `;
            historyList.appendChild(item);
        });
    } catch (error) {
        console.error('獲取預約歷史失敗:', error);
    }
}
