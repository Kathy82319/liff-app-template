...
function renderNewsList(newsItems) {
    ...
    // 防呆：如果真的讀不到，給一個安全的預設值，而不是崩潰
    if (!Array.isArray(columnsSetting) || columnsSetting.length === 0) {
        console.warn("[News] Columns config missing, using defaults.");
        columnsSetting = [
...
function setupEventListeners() {
    ...
    if (editNewsForm && !editNewsForm.dataset.listenerAttached) {
        editNewsForm.addEventListener('submit', async (e) => {
            ...
            try {
                if (newsId) await api.updateNews(formData);
...
export const init = async () => {
    ...
    try {
        ...
        // --- 核心修正：移除舊版的 throw Error 檢查，改為寬容模式 ---
