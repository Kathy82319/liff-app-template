...
export async function onRequest(context) {
    try {
        ...
        const payload = {
            email: `${userId.substring(0, 10)}@magic.link`,
        ...
        const jwt = await new jose.SignJWT(payload)
            .setProtectedHeader({ alg: 'HS256', kid: context.env.CF_ACCESS_KEY_ID })
            .sign(secret);
            
        // ▼▼▼ 【核心修改】在 Magic Link 後面加上 &demo=true ▼▼▼
        const url = new URL(context.request.url);
        const magicLink = `https://${url.hostname}/admin-panel.html?cf_authorization=${jwt}&demo=true`;
