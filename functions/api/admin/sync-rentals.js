// functions/api/admin/sync-rentals.js
...
async function runRentalSync(env) {
    ...
    const RENTALS_SHEET_NAME = '桌遊租借者'; // 直接定義或從 env 讀取

    const { results } = await DB.prepare('SELECT * FROM Rentals ORDER BY created_at DESC').all();
    if (!results || results.length === 0) {
        return { success: true, message: '資料庫中沒有租借紀錄可同步。' };
    }
    ...
    await doc.loadInfo();
    const sheet = doc.sheetsByTitle[RENTALS_SHEET_NAME];
    if (!sheet) throw new Error(`在 Google Sheets 中找不到名為 "${RENTALS_SHEET_NAME}" 的工作表。`);
    ...
    return { success: true, message: `成功將 ${results.length} 筆租借紀錄從 D1 同步至 Google Sheet。` };
}
...
export async function onRequest(context) {
    try {
        ...
        const result = await runRentalSync(context.env);
        return new Response(JSON.stringify(result), { status: 200, headers: { 'Content-Type': 'application/json' } });
    } catch (error) {
        console.error('Error in sync-rentals API:', error);
